...
yyinput, yyoutput : TextFile;        (* input and output file *)
...
procedure yyscan;
  begin
    if Length(yytext) < 1 then exit;
    if yyleng=255 then fatal('yytext overflow');
    yyactchar := get_char();
    inc(yyleng);
    ShowMessage('---> ' + yytext + #10 + IntToStr(yyleng) + #10 + IntToStr(Length(yytext)));
...
//  assign(yyinput , 'tmp.tmp');
//  assign(yyoutput, 'tmp.out');
